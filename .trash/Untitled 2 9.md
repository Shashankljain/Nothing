```mermaid

flowchart TD
Disease / RUQ Pain] ::: primary --&gt; B[Initial Imaging: Abdominal Ultrasound USG] ::: primary B --&gt; C{USG Findings} ::: warning C --&gt;|Gallstones in Gallbladder| D[Cholelithiasis Identified] ::: primary C --&gt;|CBD Dilatation / CBD Stone| E[Obstructive Biliary Disease] ::: danger D --&gt; F{Symptomatic Status} ::: warning F --&gt;|Symptomatic / Cholecystitis| G[Indication for Surgery] ::: danger F --&gt;|Asymptomatic| H[Conservative Monitoring] ::: success G --&gt; I[Laparoscopic Cholecystectomy] ::: success E --&gt; K[Non-Invasive Imaging: MRCP / EUS] ::: primary K --&gt; L[Therapeutic Intervention: ERCP] ::: primary L --&gt; M[Sphincterotomy &amp; Basket Retrieval / Stenting] ::: success M --&gt; N[Elective Laparoscopic Cholecystectomy] ::: success 
```


```mermaid
flowchart TD

classDef primary fill:#e3f2fd,stroke:#1565c0,stroke-width:2px
classDef danger fill:#ffebee,stroke:#c62828,stroke-width:2px
classDef success fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
classDef warning fill:#fff8e1,stroke:#f57f17,stroke-width:2px

A[Suspected Gallstone Disease / RUQ Pain]:::primary
--> B[Initial Imaging: Abdominal Ultrasound USG]:::primary

B --> C{USG Findings}:::warning

C -->|Gallstones in Gallbladder| D[Cholelithiasis Identified]:::primary
C -->|CBD Dilatation / CBD Stone| E[Obstructive Biliary Disease]:::danger

D --> F{Symptomatic Status}:::warning

F -->|Symptomatic / Cholecystitis| G[Indication for Surgery]:::danger
F -->|Asymptomatic| H[Conservative Monitoring]:::success

G --> I[Laparoscopic Cholecystectomy]:::success

E --> K[Non-Invasive Imaging: MRCP / EUS]:::primary
K --> L[Therapeutic Intervention: ERCP]:::primary
L --> M[Sphincterotomy & Basket Retrieval / Stenting]:::success
M --> N[Elective Laparoscopic Cholecystectomy]:::success
```